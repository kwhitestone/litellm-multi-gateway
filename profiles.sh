...
cmd_switch() {
  ...
  echo
  echo "提示："
  # 从 profile 头部注释读切换建议，没有就给默认
  local tip
  tip=$(grep -m1 '^# tip:' "$target" 2>/dev/null | sed 's/^# tip: //') || true
  if [ -n "$tip" ]; then
    echo "  $tip"
  else
    echo "  docker compose up -d   （带视觉用 --profile vision + BASE_URL :4001）"
  fi
}
...
# ---------- 子命令：new ----------
cmd_new() {
  local name="" model="" base="" keyenv="" proto="anthropic"
...

  {
    ...
    echo "# tip: docker compose up -d   Claude Code BASE_URL = http://127.0.0.1:4000"
    echo "---"
    echo "model_list:"
    printf '  - model_name: %s\n' "$model"
